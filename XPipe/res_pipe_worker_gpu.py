            ...
            if dist.get_rank() == 0:
                ...
                try:
                    ...
                    for count in range(package_size):
                        ...
                        output_v = layer(input_v.cuda())
                        package[count] = output_v.cpu()
                ...
                for count in range(package_size):
                    package[count] = layer(rec_val[count].cuda()).cpu()
                ...
                try:
                    ...
                    for count in range(package_size):
                        ...
                        target_v_pack[count] = target_temp.cuda()


                    for count in range(package_size):
                        input_v = rec_pack[count].cuda()
                        ...
                        package[count] = input_v.grad.cpu()
                ...
                try:
                    ...
                    package = torch.zeros([package_size, batch_size, 256, 8, 8], requires_grad=True)
                    for count in range(package_size):
                        input_v = input_v_pack[count].cuda()
                        input_v.requires_grad_()
                        back_grad = back_grad_pack[count].cuda()# requires_grad == True????????
                        output_v = layer(input_v)
                ...
                try:
                    ...
                    for count in range(package_size):
                        input_v = input_v_pack[count].cuda()
                        input_v.requires_grad_()
                        back_grad = back_grad_pack[count].cuda()
                        output_v = layer(input_v)
